...
// ─────────────────────────────────────────────────────────────────────────────
// MINI-INEC 2.0 — Overview Dashboard (Main Landing)
// Shows national stats, state-by-state breakdown, and PU status table
// ─────────────────────────────────────────────────────────────────────────────
...
function StateSummaryRow({ state }: { state: typeof MOCK_STATES[0] }) {
  ...
  return (
    <tr>
      ...
      <td>
        <span style={{ color: 'var(--status-active)', fontFamily: 'var(--font-mono)', fontSize: 13 }}>
          {state.stats.activePUs.toLocaleString()}
        </span>
      </td>
      <td>
        <span style={{ color: 'var(--status-offline)', fontFamily: 'var(--font-mono)', fontSize: 13 }}>
          {state.stats.offlinePUs.toLocaleString()}
        </span>
      </td>
      <td>
        <span style={{ color: 'var(--status-completed)', fontFamily: 'var(--font-mono)', fontSize: 13 }}>
          {state.stats.completedPUs.toLocaleString()}
        </span>
      </td>
      <td>
        <span style={{ color: 'var(--status-flagged)', fontFamily: 'var(--font-mono)', fontSize: 13 }}>
          {state.stats.flaggedPUs}
        </span>
      </td>
      <td>
      ...
      <td>
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--text-primary)' }}>
          {state.stats.totalVotesCast.toLocaleString()}
        </span>
      </td>
    </tr>
  )
  ...
  return (
    <div className="panel" style={{ marginTop: 20, overflow: 'hidden' }}>
      <div className="panel-title">
        <Activity size={12} />
        Polling Unit Status ({filtered.length} of {pus.length})
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 16 }}>
        <input
          className="input"
          placeholder="Search PU code, name, officer..."
          value={search}
        ...
        <select
          className="select"
          value={statusFilter}
          onChange={(e) => setStatusFilter(e.target.value as PUStatus | 'all')}
          style={{ width: 160 }}
        >
        ...
        <table className="data-table">
          ...
          <tbody>
            {filtered.map((pu) => (
              <tr key={pu.puId}>
                <td>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--green-inec)' }}>
                    {pu.puCode}
                  </span>
                </td>
                ...
                <td>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                    {pu.hasGuaranteedNetwork
                      ? <Wifi size={12} color="var(--status-active)" />
                      : <WifiOff size={12} color="var(--status-offline)" />
                    }
                    <span style={{ fontSize: 10, fontFamily: 'var(--font-mono)', color: 'var(--text-secondary)' }}>
                      {pu.networkType.toUpperCase()}
                    </span>
                  </div>
                </td>
                <td>
                ...
                <td>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12 }}>
                    {pu.accreditedVoters.toLocaleString()}
                  </span>
                </td>
                <td>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--green-inec)' }}>
                    {pu.totalVotesCast.toLocaleString()}
                  </span>
                </td>
                <td>
                  {pu.isFlagged && (
                    <Flag size={14} color="var(--severity-critical)" />
                  )}
                </td>
              </tr>
            ))}
...
// ─── Chart: State Vote Breakdown ──────────────────────────────────────────────
function StateVoteChart() {
  const data = MOCK_STATES.map((s) => ({
    name: s.code,
    votes: Math.floor(s.stats.totalVotesCast / 1000),
    active: s.stats.activePUs,
    offline: s.stats.offlinePUs,
  }))
      ...
      <ResponsiveContainer width="100%" height={140}>
        <BarChart data={data} margin={{ top: 0, right: 0, left: -20, bottom: 0 }}>
          ...
          <Tooltip
            ...
            formatter={(value: number) => [`${value}k votes`, 'Total']}
          />
          <Bar dataKey="votes" radius={[4, 4, 0, 0]}>
            {data.map((_, i) => (
              <Cell key={i} fill={i % 2 === 0 ? 'var(--green-inec)' : 'var(--green-dim)'} />
            ))}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
...
export default function OverviewPage() {
  ...
  // Filter PUs based on hierarchy selection
  ...
  return (
    <div style={{ height: '100%', overflowY: 'auto', padding: 20 }}>
        ...
        <div>
          ...
          <p style={{ fontSize: 12, color: 'var(--text-secondary)', marginTop: 2 }}>
            {filters.selectedStateId
              ? `Drilling into ${selectedState?.lgas.length} LGAs`
              : `Monitoring ${NATIONAL_STATS.totalPUs.toLocaleString()} Polling Units across ${NATIONAL_STATS.totalStates} States`
            }
          </p>
        </div>
        <button
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: 12, marginBottom: 20 }}>
        <StatCard
          label="Active PUs"
          value={'activePUs' in stats ? stats.activePUs : 0}
          icon={Activity}
        ...
        <StatCard
          label="Offline PUs"
          value={'offlinePUs' in stats ? stats.offlinePUs : 0}
          icon={WifiOff}
          color="var(--status-offline)"
          sublabel="Sync pending"
        />
        <StatCard
          label="Completed PUs"
          value={'completedPUs' in stats ? stats.completedPUs : 0}
          icon={CheckCircle}
        ...
        <StatCard
          label="Votes Cast"
          value={'totalVotesCast' in stats ? stats.totalVotesCast : 0}
          icon={BarChart3}
          color="var(--green-inec)"
          sublabel="Total national count"
        />
        {'totalVehicles' in stats && (
          <StatCard
            label="Vehicles In-Transit"
            value={stats.vehiclesInTransit}
            icon={Truck}
            color="var(--status-submitted)"
            sublabel={`of ${stats.totalVehicles.toLocaleString()} deployed`}
          />
        )}
      </div>
          ...
          <div className="panel" style={{ overflow: 'hidden' }}>
            ...
            <div style={{ overflowX: 'auto', maxHeight: 140, overflowY: 'auto' }}>
              <table className="data-table">
                ...
                <tbody>
                  {MOCK_STATES.map((s) => (
                    <StateSummaryRow key={s.stateId} state={s} />
                  ))}
                </tbody>
              </table>
            </div>
